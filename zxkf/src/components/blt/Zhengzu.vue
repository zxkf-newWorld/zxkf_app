<template>
  <div class="zhengzu">
    ...
    <zselect
      ...
      :style="zselectStyle"
      ref="zselect"
      :zselectShow.sync="zselectShow"
      :tabList[0].sync="tabList[0]"
      :tabList[1].sync="tabList[1]"
      :tabList[2].sync="tabList[2]"
      :tabList[3].sync="tabList[3]"
      :tabChangedList.sync="tabChangedList"
    ></zselect>
    <!-- 返回顶部ToTop组件 -->
    <totop></totop>
    <div class="search-choice">
      ...
      <div class="search-header">
        ...
        <!-- 隐藏显示的城市选择列表 -->
        <city
          :cityShow.sync="cityShow"
          :cityChecked.sync="cityChecked"
          v-model="cityShow"
        ></city>
    ...
    <mt-popup class="sort-container" v-model="sortShow" position="bottom">
      <ul class="sortBox">
        <li
          @click="chooseSort(i)"
          :class="{ active: i == sortIndex }"
          v-for="(item, i) of sortList"
          :key="i"
        >
          {{ item }}
        </li>
      ...
      <div class="house-card">
        ...
        <div class="extra-info">
          ...
          <div class="price-info">
            <div class="price"><span>￥1350</span>/月</div>
            <div class="use-time">
              <span>&nbsp;随时入住</span>
            </div>
          </div>
...
export default {
  ...
  created() {
    console.log("整租列表页面", this.name);
    this.searchHouseList(1, "index/fullrent");
  },
  computed: {
    ...mapGetters(["status" /* 用户登录状态 */, "myselect" /* foot-tab切换 */])
  },
  methods: {
    ...mapMutations({
